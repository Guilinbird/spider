...
class SPIDER(object):
    ...
    def __init__(self):
        self.start_url = "http://www.kanshu8.net/book/24622/"
        self.headers = {
    ...
    def first_prase(self, meitu_response):
        ...
        total_item_num = meitu_html.xpath("//div[@class='pt-chapter-cont-detail full']/a")
        if len(total_item_num) != 0:
            for i in total_item_num:
                if int(re.match(r'第(\d+)', i.text, re.S).group(1)) <= 336:
                    continue
                next_url = i.xpath("./@href")[0]
                next_url = "http://www.kanshu8.net" + next_url
                content_response = self.send_request(next_url)
                per_page = etree.HTML(content_response)
                per_line = per_page.xpath("//div[@class='size16 color5 pt-read-text']/p")
                print('准备写入%s' % i.text)
                self.save_txt([x.text for x in per_line])
        else:
            print('未读取到相关标签内容')
    ...
    def save_txt(self, line):
        if len(line) != 0:
            with open("D:\spider_data\逍遥战神.txt", "a") as f:
                for i in line:
                    f.write(i+"\n")
                f.write("\n")
    ...
    def run(self):
        ...
        self.first_prase(meitu_response)
